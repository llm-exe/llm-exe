...
>
> **Timeout on the escalated path:** the raised 65536 ceiling lets an `xhigh` run generate for much longer, and llm-exe does not stream, so the whole response must complete within `timeout` (default 30000ms). A long generation can exceed it, and a timeout ends the call without retrying. The provider may still generate and bill the original request. Raise `timeout` when using `effort: "high"` on Opus 4.7 / 4.8.
>
